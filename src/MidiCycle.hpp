...
public:
  MidiCycle(int max_length, t_outlet *outlet)
      : m_seq_recorder(max_length), m_playing{false}, m_step{0},
        ...
        m_playing_notes(), m_outlet{outlet}, m_quantize(0) {}
...
void MidiCycle::note_event(note_id note, char velocity) {
  ...
  if (velocity) {
    ...
    if (iter != m_held_notes.end()) {
      ...
      auto duration = static_cast<short>(m_step_global - note_on_info.start);
  ...
  if (m_playing) {
    ...
    if (not m_seq_recorder.step_empty(m_step)) {
      ...
      for (auto &note : tstep) {
        ...
        m_playing_notes.insert(
            {m_step_global + (long long)note.duration, note.note});
      }
    }
  } else {
    // Clear this step
